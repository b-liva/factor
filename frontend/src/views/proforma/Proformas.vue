<template>
    <div class="proformas">
        ...
        <h1 class="subheading grey--text">پیش فاکتور</h1>

        <proformaPopup @proforma_added="profAdd"></proformaPopup>
        <v-container fluid class="my-5">
            ...
            >
                ...
                <v-flex xs2>
                    <v-btn color="success" @click="getProformas">جستجو</v-btn>
                </v-flex>
                <v-flex>
                    <template>
                        <v-progress-linear
                                :indeterminate="flags.proformasLoading || flags.specsLoading"></v-progress-linear>
                    </template>
                </v-flex>
                <v-flex v-if="response.length != 0">
                    ...
                    >
                        <template v-slot:items="props">
                            <tr @click="expandProformas(props)">
...
<script>
    ...
    export default {
        ...
        props: {},
        created() {
            this.debouncedGetData = _.debounce(this.getProformas, 700);
        },
        beforeCreate() {
        },
        beforeMount() {
        ...
        methods: {
            ...
            getProformas: function () {
                this.flags.proformasLoading = true;
                this.msg = '';

                const url = 'api/proformas/index';
                let params = {
                ...
                axios.post(url, params)
                    .then((result) => {
                        console.log(result);
                        this.response = result.data.profs;
                        ...
                        this.days = result.data.diff_days;
                        this.flags.proformasLoading = false;
            ...
            expandProformas: function (props) {
                // if (!props.expanded){
                //     this.specs = '';
                // }
