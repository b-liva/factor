...
class TestIncomes(GraphQLTestCase, CustomAPITestCase):
    ...
    def test_income_rows_mutation_success(self):
        ...
        self.sample_prefspec(proforma=self.proforma, owner=self.ex_user, reqspe=spec1, price=12350000)
        self.sample_prefspec(proforma=self.proforma, owner=self.ex_user, reqspe=spec2, price=25000000)
    ...
    def test_income_row_mutation_fails_if_sum_gt_income_amount(self):
        ...
        self.sample_prefspec(proforma=prof, owner=self.ex_user, reqspe=spec1, price=12350000)
        self.sample_prefspec(proforma=prof, owner=self.ex_user, reqspe=spec2, price=25000000)

        income = self.sample_income_new(number=9809, amount=20000000)

        self.sample_income_row(income=income, proforma=prof, amount=10000000)
        self.sample_income_row(income=income, proforma=prof, amount=2000000)
        self.sample_income_row(income=income, proforma=prof, amount=3000000)
    ...
    def test_income_row_mutation_fails_if_sum_gt_proforma_amount(self):
        """Test income row mutation fails if sum of income rows is greater than proforma amount"""
        req = self.sample_request(owner=self.ex_user, customer=self.customer, number=982020)
        spec1 = self.sample_reqspec(req_id=req, owner=self.ex_user, **self.specs_payload[0])
        spec2 = self.sample_reqspec(req_id=req, owner=self.ex_user, **self.specs_payload[1])
        prof = self.sample_proforma(req=req, number=984040, owner=self.ex_user)

        self.sample_prefspec(proforma=prof, owner=self.ex_user, reqspe=spec1, price=1235000)
        self.sample_prefspec(proforma=prof, owner=self.ex_user, reqspe=spec2, price=2500000)

        income = self.sample_income_new(number=9809, amount=20355750)

        self.sample_income_row(income=income, proforma=prof, amount=2035750)
