...
from django.db.models import Sum, F, FloatField, DateField, TextField, Func
...
class CustomRequestQuerySet(models.query.QuerySet):
    def with_date(self):
        return self.annotate(
            # date_fn=Cast(jdatetime.date.fromgregorian(date=F('pub_date'), local='fa_IR'), output_field=jmodels.jDateField()),
            custom_date=CalcDate(F('date_fa'), output_field=DateField()),
            # kw_sum=ExpressionWrapper(Sum(F('reqspec__kw') * F('reqspec__qty')), output_field=FloatField())
        )
...
class Requests(models.Model):
    ...
    cobjects = CustomRequestQuerySet.as_manager()
